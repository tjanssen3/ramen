...
func (mwu *MWUtil) DeleteManifestWorksForCluster(clusterName string) error {
	...
	Uncomment this section if we decide against this later. TJanssen 2021/09/23 - Issue #240
	// Namespace
	err = mwu.deleteManifestWorkWrapper(clusterName, MWTypeNS)
	if err != nil {
		mwu.Log.Error(err, "failed to delete MW for Namespace")

		return fmt.Errorf("failed to delete ManifestWork for Namespace in namespace %s (%w)", clusterName, err)
	}
	*/
