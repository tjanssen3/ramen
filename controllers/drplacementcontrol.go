...
func (d *DRPCInstance) ensureNamespaceExistsOnManagedCluster(homeCluster string) error {
	...
	if !namespaceExists { // attempt to create it
		if err := d.mwu.CreateOrUpdateNamespaceManifest(d.instance.Name, d.instance.Namespace, homeCluster); err != nil {
	...
	return nil // either namespace exists, or we created it
}
...
func (d *DRPCInstance) namespaceExistsOnManagedCluster(cluster string) (bool, error) {
	...
	namespace, err := d.reconciler.MCVGetter.GetNamespaceFromManagedCluster(d.instance.Name, cluster, d.instance.Namespace)
	if err != nil {
	...
	// sanity checks: only allow Active namespaces
	if namespace == nil {
		return !exists, errorswrapper.Wrap(err, "failed to get Namespace from managedcluster - namespace=nil")
	}

	if namespace.Status.Phase != corev1.NamespaceActive {
		return !exists, errorswrapper.Wrap(err, "namespace.Status.Phase != NamespaceActive")
	}
