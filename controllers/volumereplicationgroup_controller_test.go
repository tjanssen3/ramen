...
var _ = Describe("Test VolumeReplicationGroup", func() {
	...
	Context("in primary state", func() {
		...
		It("change to secondary", func() { // should already be "primary"
			...
			pvcs, err := rmnutil.GetMetricValueSingle("ramen_vrg_timer_to_secondary", dto.MetricType_HISTOGRAM, metrics.Registry)

			Expect(err).NotTo(HaveOccurred())
			Expect(pvcs).To(Equal(float64(len(vrgTests))))
		})
		It("cleans up after testing", func() {
			for c := 0; c < len(vrgTests); c++ {
				v := vrgTests[c]
...
func (v *vrgTest) removeFinalizerFromPVCs(finalizer string) {
	for _, pvcName := range v.pvcNames {
		...
		if listContainsString(pvc.ObjectMeta.Finalizers, finalizer) {
			pvc.ObjectMeta.Finalizers = removeStringFromList(pvc.ObjectMeta.Finalizers, finalizer)
		}
...
func removeStringFromList(values []string, s string) []string {
	result := []string{}
...
func listContainsString(values []string, s string) bool {
	for _, item := range values {
		if item == s {
			return true
		}
