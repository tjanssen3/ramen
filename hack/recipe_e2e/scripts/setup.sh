...
# TODO: check minikube IP matches ramen config
...
# mc buckets
if [[ $(mc ls minio-cluster1 | grep velero | wc -l) -eq 0 ]]; then
  mc mb minio-cluster1/velero
fi

if [[ $(mc ls minio-cluster2 | grep velero | wc -l) -eq 0 ]]; then
  mc mb minio-cluster2/velero
fi

# backube
if [[ $(kubectl get crd | grep replicationdestinations.volsync.backube | wc -l) -eq 0 ]]; then
...
if [[ $(kubectl get crd | grep replicationsources.volsync.backube | wc -l) -eq 0 ]]; then
